#!/usr/bin/env python3
"""
HiFi Crossfader with Matched Gain
- Apply same preamp gain to both dry and wet signals
- Perfect level matching for fair comparison
"""
...
class HiFiMatchedGainCrossfader:
    def __init__(self, audio_file, target_phon=60, reference_phon=80, 
        ...
        print(f"  Level correction needed for wet: {self.correction_db:+.2f} dB")
        print(f"  \U0001f4a1 Applying SAME gain to BOTH dry and wet signals")
        print(f"  This ensures fair A/B comparison at equal loudness")
        
        # Create delay buffer for dry signal
        ...
        print(f"\n\U00002728 MATCHED GAIN MODE: Both signals at {self.correction_db:+.2f} dB")
    ...
    def process_wet_block(self, block):
        """Apply FIR filter (wet signal)"""
        ...
        # Combine channels (NO level correction here - will apply to both)
        filtered = np.column_stack([filtered_l, filtered_r])
        
        return filtered
        
    def audio_callback(self, outdata, frames, time, status):
        """Audio callback with matched gain on both signals"""
        if status:
        ...
        wet_block = self.process_wet_block(dry_block)
        
        # Delay dry signal to match FIR delay
        dry_block_delayed = self.delay_dry_signal(dry_block)
        
        # Apply SAME gain to BOTH signals
        dry_block_delayed *= self.correction_linear
        wet_block *= self.correction_linear
        ...
        # Mix signals (both have same gain applied)
        mixed = dry_block_delayed * gain_dry + wet_block * gain_wet
        
        # Soft limiting to prevent clipping
        threshold = 0.95
        mask = np.abs(mixed) > threshold
        if np.any(mask):
            over = np.abs(mixed[mask]) - threshold
            mixed[mask] = np.sign(mixed[mask]) * (threshold + np.tanh(over * 2) * 0.05)
    ...
    def start(self):
        ...
        self.stream.start()
        print("\n\U0001f3b5 HiFi Playback Started with MATCHED GAIN!")
        print("\nControls:")
        print("  1-9 : Crossfade (1=original, 9=90% filtered)")
        print("  0   : 100% filtered")
        print("  r   : Reset to original")
        print("  q   : Quit")
        print(f"\n\U00002705 Both signals at {self.correction_db:+.2f} dB for fair comparison")
        print("\n")
    ...
    def set_crossfade(self, value):
        ...
        if self.target_crossfade == 0:
            state = f"ORIGINAL (matched gain {self.correction_db:+.1f}dB)"
        elif self.target_crossfade == 1:
            state = f"FILTERED (matched gain {self.correction_db:+.1f}dB)"
        else:
...
def main():
    parser = argparse.ArgumentParser(
        description='HiFi crossfader with matched gain on both signals',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Matched Gain Mode:
  - Same preamp gain applied to BOTH dry and wet signals
  - This ensures fair A/B comparison at equal loudness
  - Only the frequency response differs between signals

Example:
  %(prog)s music.flac
  %(prog)s music.flac --taps 8191    # Ultra high quality
...
""")
    ...
    try:
        crossfader = HiFiMatchedGainCrossfader(
            args.audio_file,
