/**
 * Adaptive Loudness Processor
 * ISO 226 기반 실시간 등청감 보정 처리기
 * 
 * 인지적 보상을 고려한 40% 규칙 적용
 */
...
class AdaptiveLoudnessProcessor {
    constructor(sampleRate = 48000, options = {}) {
        ...
        this.options = {
            taps: 513,  // FIR 필터 탭 수
            window: 'hann',
            perceptualCompensation: 0.4,  // 인지적 보상 40%
            smoothingFactor: 0.98,  // 필터 전환 스무딩
            adaptiveTimeConstant: 300,  // 5분 (초)
            ...options
        };
    ...
    updateEnvironment(noiseFloorDB, playbackLevelDB) {
        ...
        const newReferencePhon = playbackLevelDB + 5;  // +5 phon 보상
    ...
    processBuffer(inputBuffer, outputBuffer) {
        ...
        for (let ch = 0; ch < numChannels; ch++) {
            ...
            // FIR 필터 적용 (컨볼루션)
            this.applyFIR(input, output, numSamples);
        }
    }
     ...
     * FIR 필터 적용
     */
    applyFIR(input, output, numSamples) {
        const coeffs = this.currentCoeffs;
        const taps = coeffs.length;
        const delay = Math.floor(taps / 2);
        
        // 이전 샘플 버퍼 (순환 버퍼로 구현 가능)
        if (!this.delayBuffer) {
            this.delayBuffer = new Float32Array(taps);
        }
        ...
        for (let n = 0; n < numSamples; n++) {
            ...
            for (let k = 0; k < taps; k++) {
                ...
                if (idx >= 0) {
                    ...
                    // 이전 버퍼에서 가져오기
                    sum += this.delayBuffer[taps + idx] * coeffs[k];
                }
            }
        ...
        for (let i = 0; i < Math.min(taps, numSamples); i++) {
            this.delayBuffer[i] = input[copyStart + i];
        }
    }
...
// Web Audio API 통합 예제
class AdaptiveLoudnessNode extends AudioWorkletProcessor {
    constructor() {
        super();
        this.processor = new AdaptiveLoudnessProcessor(sampleRate);
        
        // 파라미터 업데이트 리스너
        this.port.onmessage = (event) => {
            if (event.data.type === 'updateEnvironment') {
                this.processor.updateEnvironment(
                    event.data.noiseFloor,
                    event.data.playbackLevel
                );
            }
        };
    }
    ...
    process(inputs, outputs, parameters) {
        ...
        // 가상 버퍼 객체 생성 (Web Audio API 호환)
        const inputBuffer = {
            numberOfChannels: input.length,
            length: input[0].length,
            getChannelData: (ch) => input[ch]
        };
        
        const outputBuffer = {
            numberOfChannels: output.length,
            length: output[0].length,
            getChannelData: (ch) => output[ch]
        };
        
        this.processor.processBuffer(inputBuffer, outputBuffer);
...
if (typeof registerProcessor !== 'undefined') {
    registerProcessor('adaptive-loudness-processor', AdaptiveLoudnessNode);
}
